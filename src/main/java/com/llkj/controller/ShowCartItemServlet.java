package com.llkj.controller;

import com.llkj.bean.Cart;
import com.llkj.bean.CartItem;
import com.llkj.utils.JSONUtils;
import com.llkj.utils.ResultJson;
import com.llkj.utils.ViewServlet;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;
import java.io.IOException;
import java.util.HashMap;
import java.util.List;

/**
 * @ClassName ShowCartItemServlet
 * @Description TODO
 * @Author qing
 * @Date 2022/11/15 18:17
 * @Version 1.0
 */
@WebServlet("/pages/cart/getcartlist")
public class ShowCartItemServlet extends ViewServlet {
    @Override
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ResultJson resultJson=null;
        HttpSession session = null;
        try {
            session = req.getSession();
            Cart mycart = (Cart)session.getAttribute("mycart");
            List<CartItem> cartItemList = mycart.getCartItemList();
            HashMap<Object, Object> resultMap = new HashMap<>();
            if (mycart!=null){
                resultMap.put("totalAmount",mycart.getTotalAmount());//消费总金额
                resultMap.put("totalCount",mycart.getTotalCount());//购买总量
                resultMap.put("cartitemlist",mycart.getCartItemList());//购物商品的集合
            }
            resultJson = ResultJson.ok().setResultData(resultMap);
        } catch (Exception e) {
            e.printStackTrace();
            resultJson=ResultJson.error();
            resultJson.setResultData(null);
        }
        JSONUtils.writeResult(resp,resultJson);
    }
}
